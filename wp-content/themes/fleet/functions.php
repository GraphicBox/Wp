...
add_action('admin_menu' , 'brdesign_enable_pages2'); 
 
function brdesign_enable_pages2() {
    add_submenu_page('index.php', 'API ID', 'API ID', 'manage_options', basename(__FILE__), 'arch_options');
}
function arch_options(){
	?><div style="width:100%;">
    <h1>Template IDs</h1>
    <?php $all = get_info('', 'even');
	foreach($all as $one):
	?>
    <div style="width:300px;float:left;"><strong><?php echo $one->name;?></strong></div>  <?php echo $one->id;?><br>
    <?php endforeach;?>
  </div>
<?php
}
